...
export function setupRoutes(app: Express): void {
  ...
  app.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const userRole =
        roleStr === 'host' ? RtcRole.PUBLISHER : RtcRole.SUBSCRIBER
      ...
      try {
        ...
        logger.info(
          `Generated Agora token for channel: ${channelNameStr}, uid: ${uidStr}, role: ${roleStr}`
        )
  ...
  app.get(
    '/api/livestreams/:liveId/viewers',
    asyncHandler(async (req: Request, res: Response) => {
      const { liveId } = req.params
      const redisService = getRedisService()
