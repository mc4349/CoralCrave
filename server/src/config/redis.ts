...
export async function initializeRedis(): Promise<void> {
  try {
    ...
    logger.warn('Redis not available, falling back to in-memory storage:', error instanceof Error ? error.message : String(error))
...
export function getRedisService(): RedisService | InMemoryService {
  if (!cacheService) {
    throw new Error('Cache service not initialized. Call initializeRedis() first.')
  }
  return cacheService
}
