      ...
      <div className="flex flex-col lg:flex-row h-screen">
        ...
        <div className="flex-1 flex flex-col relative">
          ...
          <div className="flex-1 bg-slate-900 relative rounded-lg m-2 overflow-hidden border border-slate-700/50">
            ...
            {/* Stream info overlay - Mobile optimized */}
            <div className="absolute top-2 left-2 right-2 sm:top-4 sm:left-4 sm:right-auto">
              <div className="bg-slate-800/90 backdrop-blur-sm rounded-lg p-2 sm:p-3 text-white border border-slate-700/50">
                <div className="flex items-start justify-between space-x-2 sm:space-x-4">
                  <button
                    onClick={handleSellerClick}
                    className="text-left hover:bg-slate-700/50 rounded p-1 -m-1 transition-colors flex-1 min-w-0"
                  >
                    <h2 className="text-sm sm:text-lg font-semibold hover:text-cyan-300 transition-colors truncate">
                      {currentStream?.hostUsername || 'Coral Seller'}
                    </h2>
                    <div className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-slate-300">
                      <div className="flex items-center space-x-1">
                        <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M10 12a2 2 0 100-4 2 2 0 000 4z"/>
                          <path fillRule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd"/>
                        </svg>
                        <span className="truncate">{viewerCount} viewer{viewerCount !== 1 ? 's' : ''}</span>
                      </div>
                      <span className="hidden sm:inline">•</span>
                      <span className="hidden sm:inline truncate">Click to view profile</span>
                    </div>
                  </button>
                  ...
                  {currentUser && currentStream?.hostId && currentUser.uid !== currentStream.hostId && (
                    <button
                      ...
                      className={`px-2 py-1 sm:px-3 rounded text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
                        isFollowing
            ...
            {currentStream?.status === 'live' && (
              <div className="absolute top-2 right-2 sm:top-4 sm:right-4">
                <div className="flex items-center space-x-1 sm:space-x-2 bg-gradient-to-r from-red-500 to-red-600 text-white px-2 py-1 sm:px-3 rounded-full shadow-lg">
          ...
          <div className="h-48 sm:h-64 m-2 mt-0">
            <LiveChat liveId={id || ''} />
          </div>
        </div>

        {/* Auction Panel - Mobile responsive */}
        <div className="w-full lg:w-80 m-2 mt-0 lg:ml-0 lg:mt-2">
          <AuctionPanel
            liveId={id || ''}
