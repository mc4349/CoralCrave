...
export default function AuctionPanel({
    ...
    const unsubscribe = onSnapshot(auctionQuery, snapshot => {
      if (!snapshot.empty) {
        ...
        if (auctionData.endTime) {
          ...
          if (
            auctionData.endTime &&
            typeof auctionData.endTime === 'object' &&
            'toDate' in auctionData.endTime
          ) {
          ...
          try {
            ...
            if (highestBidDoc) {
              const highestBid = {
                ...
                ...highestBidDoc.data(),
                timestamp:
                  highestBidDoc.data().timestamp?.toDate() || new Date(),
              } as Bid
              ...
              if (highestBid.amount >= auction.startingPrice) {
                ...
                await addDoc(
                  collection(db, 'users', highestBid.userId, 'cart'),
                  cartItem
                )
                ...
                console.log(
                  `Auction ended! Winner: ${highestBid.username} with $${totalPrice} (including $${shippingCost} shipping)`
                )
              } else {
                console.log(
                  'No valid bids found - bid amount below starting price'
                )
              }
            } else {
  ...
  const placeBid = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await chatService.sendBidAnnouncement(
  ...
  const createAuction = async (e: React.FormEvent) => {
    ...
    const { title, description, startingPrice, shippingPrice, duration } =
      auctionForm
    ...
    return (
      <div className={`bg-slate-800 rounded-lg p-4 ${className}`}>
        <p className='text-slate-400 text-center'>
          Please log in to participate in auctions
        </p>
      </div>
    )
  }
        ...
        {isLoading ? (
          ...
          <div className='space-y-3'>
            ...
            <div className='space-y-1'>
              <h5 className='text-slate-300 text-sm font-semibold'>
                Recent Bids
              </h5>
              {bids.length === 0 ? (
            ...
            {winner && (
              <div className='bg-green-900/50 border border-green-700 rounded p-3'>
                <h5 className='text-green-300 text-sm font-semibold'>
                  🎉 Auction Ended!
                </h5>
              ...
              <form onSubmit={createAuction} className='space-y-4'>
                ...
                <div className='flex flex-col sm:flex-row gap-3 pt-4'>
                  ...
                  <button
                    type='submit'
                    disabled={
                      !auctionForm.title.trim() ||
                      !auctionForm.description.trim() ||
