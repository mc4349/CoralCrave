...
export default function InvoiceViewer({ userType, className = '' }: InvoiceViewerProps) {
  ...
  useEffect(() => {
    ...
    const invoicesQuery = query(
      ...
      where(userType === 'buyer' ? 'buyerId' : 'sellerId', '==', currentUser.uid)
    )
    ...
    const unsubscribe = onSnapshot(invoicesQuery, (snapshot) => {
      ...
      setInvoices(invoiceList.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()))
      setIsLoading(false)
    })
  ...
  const updateTracking = async () => {
    ...
    try {
      await invoiceService.updateTrackingNumber(selectedInvoice.invoiceId, trackingNumber.trim())
      setTrackingNumber('')
  ...
  const downloadInvoice = () => {
    ...
    const blob = new Blob([selectedInvoice.htmlContent || ''], { type: 'text/html' })
  ...
  if (!currentUser) {
    return (
      <div className={`bg-slate-800 rounded-lg p-4 ${className}`}>
        <p className="text-slate-400 text-center">Please log in to view invoices</p>
      </div>
    )
  }
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {invoices.map((invoice) => (
              <div key={invoice.invoiceId} className="bg-slate-700 rounded-lg p-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <h4 className="text-white font-semibold">{invoice.itemTitle}</h4>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        invoice.type === 'buyer' ? 'bg-blue-600 text-white' : 'bg-green-600 text-white'
                      }`}>
                        {invoice.type === 'buyer' ? 'Purchase' : 'Sale'}
                      </span>
                    </div>
                    <p className="text-slate-300 text-sm mb-2">{invoice.itemDescription}</p>
                    <div className="text-sm text-slate-400 space-y-1">
                      <p>Invoice ID: {invoice.invoiceId}</p>
                      <p>Date: {invoice.createdAt.toLocaleDateString()}</p>
                      <p>Total: ${invoice.totalPrice.toFixed(2)}</p>
                      {invoice.trackingNumber && (
                        <p className="text-green-400">Tracking: {invoice.trackingNumber}</p>
                      )}
                    </div>
                  </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {selectedInvoice.htmlContent ? (
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: selectedInvoice.htmlContent }}
                />
              ) : (
                <div className="text-center text-slate-400">
                ...
                <div className="mt-6 bg-slate-700 rounded-lg p-4">
                  ...
                  {selectedInvoice.trackingNumber ? (
                    <div className="text-green-400">
                      <p className="font-medium">Tracking Number: {selectedInvoice.trackingNumber}</p>
                      <p className="text-sm text-green-300 mt-1">Tracking information has been provided to the buyer.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <p className="text-slate-300 text-sm">
                        Add a tracking number to notify the buyer when their item ships.
                      </p>
                      <div className="flex gap-2">
                        <input
                          type="text"
